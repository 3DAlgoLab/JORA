...
@jax.value_and_grad
def train_forward_lora(
    lora_params, lora_config, params: Llama, data_batch: TrainData, *, key: Array
):
    seq, seq_mask, labels, labels_mask = data_batch
    outputs = llama_model_lora(
        lora_params,
        lora_config,
        params.model,
        seq,
...
def train_step_lora(
    ...
    opt_state: Any,
    total_loss: Array,
    data_batch: TrainData,
    key: Array,
) -> tuple[Llama, Any, Array, Array, Array]:
    key, subkey = rand.split(key)
    loss, grads = train_forward_lora(
        lora_params, lora_config, params, data_batch, key=subkey
    )
    total_loss += loss
...
def generate_alpaca_dataset(
    path: str, split: str, config: ParallamaConfig, split_percentage=0.8, alpaca_mix=0.3
):
    """
...
def train_lora(
    ...
    if not config.JAX_PARAMS_PATH or not config.LLAMA2_META_PATH:
        raise ValueError(
            "Please provide JAX_PARAMS_PATH and LLAMA2_HF_PATH in the config"
        )

    if not config.MODEL_SIZE or config.MODEL_SIZE not in ["7B", "13B", "70B"]:
        raise ValueError(
            "Please provide a valid MODEL_SIZE in the config (7B, 13B, 70B)"
        )
    ...
    q_proj_shape, v_proj_shape = (
        params.model.decoder.attention.q_proj.shape,
        params.model.decoder.attention.v_proj.shape,
    )
    ...
    with jax.default_device(cpu_device):
        q_lora_A = rand.normal(
            split_qa, (DB, H, N_REP, N_HEADS, config.LORA_R), dtype=jnp.bfloat16
        )
        q_lora_B = jnp.zeros(
            (DB, config.LORA_R, N_REP, N_HEADS, D_K), dtype=jnp.bfloat16
        )
        v_lora_A = rand.normal(
            split_va, (DB, H, N_HEADS, config.LORA_R), dtype=jnp.bfloat16
        )
    ...
    for epoch in tqdm(range(config.N_EPOCHS)):
        ...
        for step, data_batch in enumerate(dataloader):
            lora_params, opt_state, total_loss, loss, key = train_step_lora(
                lora_params, loraConfig, params, opt_state, total_loss, data_batch, key
            )
