...
def merge_llama2(params_path: str, lora_path: str, output_path: str):
    ...
    q_lora = jnp.einsum(
        "dmosr,drosn->dmosn", lora_params["q_lora_A"], lora_params["q_lora_B"]
    )
    v_lora = jnp.einsum(
        "dmsr,drsn->dmsn", lora_params["v_lora_A"], lora_params["v_lora_B"]
    )

    # add the final additive params to the original params
    updated_params = tree.map_structure_with_path(
        lambda p, v: merge_q_v(p, v, q_lora, v_lora), params
    )
...
def merge_gemma(params_path: str, lora_path: str, output_path: str):
    ...
    params_dirs = [
        x
        for x in os.listdir(params_path)
        if os.path.isdir(os.path.join(params_path, x))
    ]
    ...
    for k, v in lora_params.items():
        ...
        if "kv_einsum" in k:
            ...
            w = orb_params[orb_key]["w"]
            orb_params[orb_key]["w"] = np.array(
                w + jnp.einsum("hmr,hrv->hmv", v["q_lora_A"], v["q_lora_B"])
            )
...
def merge_lora(
    params_path: str, lora_path: str, output_path: str, llama2=False, gemma=False
):
    if llama2 and gemma:
        raise ValueError(
            "Please specify only one type of model llama2 or gemma. Use -h for help."
        )
    if llama2:
        ...
        raise ValueError(
            "Please specify type of model llama2 or gemma. Use -h for help."
        )
