...
def _loop_body_top_p(
    params: Llama,
    state: _TopPGenerationState,
    model_config: ModelConfig,
    top_p_config: TopPGenerationConfig,
) -> _TopPGenerationState:
    key, seq, attn_mask, last_positions, last_tokens, finished = state
    eos_token_id, max_length, top_p = top_p_config

    outputs = llama_model(
        params.model, seq, attn_mask, key=None, model_config=model_config
    )
    ...
    indices = jnp.broadcast_to(
        jnp.arange(vocab_size, dtype=jnp.uint16), (batch_size, vocab_size)
    )
    sorted_logits, sorted_indices = jax.lax.sort_key_val(
        -logits, indices, is_stable=False
    )
    sorted_logits = -sorted_logits
    sorted_probs = nn.softmax(sorted_logits)
    cum_probs = jnp.cumsum(sorted_probs, axis=-1)
    threshold_probs = jnp.maximum(
        cum_probs[:, 0], top_p
    )  # guarantee that at least one token will not be masked
    masked_sorted_logits = jnp.where(
        cum_probs >= threshold_probs[:, None], -jnp.inf, sorted_logits
    )
    ...
    seq = jax.vmap(lambda x, y, z: x.at[y].set(z))(
        seq, current_positions, current_tokens
    )
    ...
    return _TopPGenerationState(
        key, seq, attn_mask, current_positions, current_tokens, finished
    )
...
def top_p(
    ...
    *,
    ...
    loop_body = lambda state: _loop_body_top_p(
        params, state, model_config, top_p_config
    )
    state = _TopPGenerationState(
        key, seq, attn_mask, last_positions, last_tokens, finished
    )
