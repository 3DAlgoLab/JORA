...
def check_attention(params: Attention, *, model_config: ModelConfig) -> None:
    ...
    assert params.q_proj.shape == (
        model_config.d_model,
        model_config.n_rep_kv,
        model_config.n_heads_kv,
        model_config.d_k,
    )
    assert params.k_proj.shape == (
        model_config.d_model,
        model_config.n_heads_kv,
        model_config.d_k,
    )
    assert params.v_proj.shape == (
        model_config.d_model,
        model_config.n_heads_kv,
        model_config.d_v,
    )
    assert params.out_proj.shape == (
        model_config.n_rep_kv,
        model_config.n_heads_kv,
        model_config.d_v,
        model_config.d_model,
...
def attention(
    ...
    *,
    ...
    q = op.einsum(
        src_seq,
        params.q_proj,
        "batch_size src_seq_len d_model, d_model n_rep_kv n_heads_kv d_k -> batch_size n_rep_kv n_heads_kv src_seq_len d_k",
    )
    k = op.einsum(
        dst_seq,
        params.k_proj,
        "batch_size dst_seq_len d_model, d_model n_heads_kv d_k -> batch_size n_heads_kv dst_seq_len d_k",
    )
    v = op.einsum(
        dst_seq,
        params.v_proj,
        "batch_size dst_seq_len d_model, d_model n_heads_kv d_v -> batch_size n_heads_kv dst_seq_len d_v",
    )
    ...
    qk = op.einsum(
        q,
        k,
        "batch_size n_rep_kv n_heads_kv src_seq_len d_k, batch_size n_heads_kv dst_seq_len d_k -> batch_size n_rep_kv n_heads_kv src_seq_len dst_seq_len",
    )
    ...
    qkv = op.einsum(
        qk,
        v,
        "batch_size n_rep_kv n_heads_kv src_seq_len dst_seq_len, batch_size n_heads_kv dst_seq_len d_v -> batch_size n_rep_kv n_heads_kv src_seq_len d_v",
    )

    out = op.einsum(
        qkv,
        params.out_proj,
        "batch_size n_rep_kv n_heads_kv src_seq_len d_v, n_rep_kv n_heads_kv d_v d_model -> batch_size src_seq_len d_model",
    )
    return out
